...
import org.greengin.sciencetoolkit.ui.base.dlgs.ResetUploadStatusDlg;
...
public class SeriesListFragment extends SenseItEventFragment implements
        SeriesListListener, SeriesActionListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.view_series_list, container,
                false);

        if (this.profileId != null) {
            adapter = new SeriesListAdapter(this.getActivity(), profileId,
                    this, inflater);
    ...
    public void seriesUpload(Model profile, File series) {
        ((RemoteCapableActivity) getActivity())
                .remoteRequest(new UploadRemoteAction(getActivity(), profile,
                        series));
    }
    ...
    public void seriesEdit(Model profile, File series) {
        ...
        EditTextDlg.open(this.getActivity(), R.string.series_edit_name_title,
                R.string.series_edit_name_msg, R.string.button_label_set,
                seriesName, true, new EditSeriesTitleManager(profile, series));
    }

    @Override
    public void seriesSelected(Model profile, File series) {
        ((DataViewerActivity) getActivity()).getSupportActionBar()
    ...
    public void seriesShare(Model profile, File series) {
        ...
        if (uris.size() > 0) {
            String shareMenuTitle = getResources().getString(
                    R.string.series_share_title);
            String subject = String.format(
                    getResources().getString(R.string.series_share_subject),
                    DataLogger.get().seriesName(profile, series));
    ...
    @Override
    public boolean seriesResetUpload(Model profile, File series) {
        if (DataLogger.get().uploadedStatus(profile.getString("id"), series) == 2) {
            ResetUploadStatusDlg.open(getActivity(), profile, series);
            return true;
        } else {
            return false;
        }
