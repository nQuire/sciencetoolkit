...
public abstract class AbstractXYSensorPlotFragment extends SenseItEventFragment {
	...
	public void setTitle(String title) {
		((TextView) plotPanel.findViewById(R.id.plot_header_label))
				.setTag(title);
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		plotContainer = (LinearLayout) plotPanel
				.findViewById(R.id.plot_container);

		scaleDetector = new PlotScaleGestureDetector();

		sensorBrowserContainer = plotPanel
				.findViewById(R.id.plot_sensor_browser);
		sensorBrowser = (SensorBrowserLayout) sensorBrowserContainer
				.findViewById(R.id.sensor_browser_layout);
	...
	protected void createPlot() {
		...
		plot.setRangeBottomMax(-1);
		plot.setRangeTopMin(1);
		
		plot.setDomainValueFormat(createDomainNumberFormat());

		int background = this.getActivity().getResources()
				.getColor(R.color.plot_outter_background);
		int gridBackground = this.getActivity().getResources()
				.getColor(R.color.plot_inner_background);
		int lineColor = this.getActivity().getResources()
				.getColor(R.color.plot_main_line);
		int sublineColor = this.getActivity().getResources()
				.getColor(R.color.plot_minor_line);
		int transparent = this.getActivity().getResources()
		...
		plot.getGraphWidget().getCursorLabelBackgroundPaint()
				.setColor(background);
		...
		plot.getGraphWidget().getDomainOriginLabelPaint().setTextSize(labelSize);
		plot.getGraphWidget().getRangeOriginLabelPaint().setTextSize(labelSize);
		
		plot.setDomainStep(XYStepMode.SUBDIVIDE, 8);

		plot.getLegendWidget().getPositionMetrics().getYPositionMetric()
				.set(25, YLayoutStyle.ABSOLUTE_FROM_BOTTOM);
		plot.getLegendWidget().getHeightMetric()
				.set(30, SizeLayoutType.ABSOLUTE);
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE: {
				if (scaling) {
					...
					if (pmin1y != pmax1y) {
						float nmaxy = (((miny - maxy) * pmax0y + maxy) * pmin1y
								+ (maxy - miny) * pmax1y * pmin0y - maxy
								* pmax1y)
								/ (pmin1y - pmax1y);
						float nminy = nmaxy
								+ ((miny - maxy) * pmin0y + (maxy - miny)
										* pmax0y) / (pmin1y - pmax1y);
						maxy = nmaxy;
						miny = nminy;
					...
					if (scaleX && pmin1x != pmax1x) {
						float nmaxx = (((minx - maxx) * pmax0x + maxx) * pmin1x
								+ (maxx - minx) * pmax1x * pmin0x - maxx
								* pmax1x)
								/ (pmin1x - pmax1x);
						float nminx = nmaxx
								+ ((minx - maxx) * pmin0x + (maxx - minx)
										* pmax0x) / (pmin1x - pmax1x);
						maxx = nmaxx;
						minx = nminx;
						plot.setDomainBoundaries(minx, BoundaryMode.FIXED,
								maxx, BoundaryMode.FIXED);
					}
					plot.redraw();
				}
				break;
